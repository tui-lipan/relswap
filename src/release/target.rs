...
/// Targets for which signed release assets are published.
...
impl Target {
    ...
    /// The launcher protocol this target publishes, when the app configures a Windows launcher.
    pub fn launcher_protocol(self, app: &App) -> Option<u32> {
        if self.is_windows() {
            app.launcher_protocol()
        } else {
            None
        }
    }

    /// The payload basename published for this target.
    pub fn payload_name(self, app: &App) -> String {
        app.payload_name(self.is_windows())
    }

    /// The launcher basename published for this target, when configured.
    pub fn launcher_name(self, app: &App) -> Option<&'static str> {
        if self.is_windows() {
            app.launcher_name()
        } else {
            None
    ...
    pub fn root_name(self, app: &App, version: &Version) -> String {
        app.root_name(version, self.as_str())
    }

    /// The canonical published archive filename.
    pub fn archive_name(self, app: &App, version: &Version) -> String {
        format!("{}{}", self.root_name(app, version), self.archive_suffix())
    }

    /// The canonical payload member path inside an archive.
    pub fn payload_path(self, app: &App, version: &Version) -> String {
        format!(
            "{}/{}",
            self.root_name(app, version),
            self.payload_name(app)
        )
    }

    /// The canonical optional launcher member path inside a Windows archive.
    pub fn launcher_path(self, app: &App, version: &Version) -> Option<String> {
        self.launcher_name(app)
            .map(|name| format!("{}/{}", self.root_name(app, version), name))
    }
...
mod tests {
    ...
    fn every_supported_target_has_canonical_names() {
        ...
        assert_eq!(
            Target::X86_64UnknownLinuxGnu.archive_name(&TEST_APP, &version),
            "hyprmux-1.2.3-x86_64-unknown-linux-gnu.tar.gz"
        );
        assert_eq!(
            Target::Aarch64AppleDarwin.payload_path(&TEST_APP, &version),
            "hyprmux-1.2.3-aarch64-apple-darwin/hyprmux"
        );
        assert_eq!(
            Target::X86_64PcWindowsMsvc.launcher_path(&TEST_APP, &version),
