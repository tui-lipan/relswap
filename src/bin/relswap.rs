...
//!
//! The wire formats in this file are deliberately small and boring. Keeping the tool at this
//! boundary means a release job can be built once and used without a second compiler invocation
//! after a private key has been made available.
...
use relswap::{
    ActivationStrategy, App, ReleaseAsset, ReleaseManifest, Target,
    release::{
...
const EMPTY_TRUST_ANCHOR: &[u8] = br#"{"schema_version":1,"keys":[]}"#;

type Result<T> = std::result::Result<T, String>;

fn main() {
    if let Err(error) = run() {
        eprintln!("relswap: {error}");
        std::process::exit(2);
    }
}
...
fn run() -> Result<()> {
    ...
    match args.command.as_deref() {
        Some("keygen") | Some("generate-key") | Some("rotate") => {
...
fn print_usage() {
    println!(
        "\
relswap: release metadata and signing helper\n\n\
Commands:\n\
  ...
  rotate --id ID --private-key PATH --public-key PATH\n\
      Generate a rotation key pair suitable for appending to the trust store.\n\
  manifest --name APP [--launcher-name FILE] --version VERSION --output PATH\n\
           [--artifacts-dir DIR] [--archive PATH ...] [--expires-at RFC3339]\n\
      Inspect completed canonical archives, write {{name}}-release.json, and write adjacent\n\
      .sha256 files from the final archive bytes.\n\
...
fn app_from_args(args: &Arguments) -> Result<App> {
    let name: &'static str = Box::leak(args.one("name")?.to_string().into_boxed_str());
    let launcher_name: &'static str = Box::leak(
        args.optional_one("launcher-name")?
            .map(str::to_string)
            .unwrap_or_else(|| format!("{name}-launcher.exe"))
            .into_boxed_str(),
    );
    Ok(App {
        name,
...
fn generate_manifest(args: &Arguments) -> Result<()> {
    ...
    let archives = collect_archives(args, &app)?;
    let mut targets = BTreeMap::new();
    for archive in archives {
        let probe = inspect_archive(&app, &archive, version)?;
        ...
        release_archive::inspect_archive(&app, &bytes, &asset)
    ...
    let published_at = published.to_rfc3339_opts(SecondsFormat::Secs, true);
    let expires_at = match args.optional_one("expires-at")? {
        Some(value) => {
            validate_timestamp(value, "expires_at")?;
            value.to_string()
        }
        None => (published + Duration::days(365)).to_rfc3339_opts(SecondsFormat::Secs, true),
    };
    let release_manifest =
        ReleaseManifest::new(&app, parsed_version, published_at, expires_at, targets)
            .map_err(|error| error.to_string())?;
...
fn collect_archives(args: &Arguments, app: &App) -> Result<Vec<PathBuf>> {
    ...
    if let Some(directory) = args.optional_one("artifacts-dir")? {
        ...
        for entry in entries {
            ...
            let path = entry.path();
            if path.is_file() && is_archive_name(app, &path) {
                paths.push(path);
            }
        }
    }
...
fn is_archive_name(app: &App, path: &Path) -> bool {
    path.file_name()
        .and_then(|name| name.to_str())
        .is_some_and(|name| {
            name.starts_with(&app.archive_prefix())
                && (name.ends_with(".tar.gz") || name.ends_with(".zip"))
        })
}

fn inspect_archive(app: &App, path: &Path, version: &str) -> Result<ArtifactProbe> {
    let name = path
        .file_name()
        .and_then(|name| name.to_str())
        .ok_or_else(|| format!("archive path {} is not valid UTF-8", path.display()))?;
    if !is_archive_name(app, path) {
        return Err(format!("non-canonical archive name {name:?}"));
    }
    let prefix = format!("{}{version}-", app.archive_prefix());
    ...
    let expected_payload = app.payload_name(is_windows);
    let expected_payload_path = format!("{root}/{expected_payload}");
    let expected_launcher = app
        .launcher_name()
        .ok_or_else(|| "app does not configure a Windows launcher".to_string())?;
    let expected_launcher_path = format!("{root}/{expected_launcher}");
    ...
    if is_windows && launcher.is_none() {
        return Err(format!("Windows archive {name} lacks {expected_launcher}"));
    }
...
fn sign_manifest(args: &Arguments) -> Result<()> {
    ...
    validate_timestamp(&manifest.expires_at, "expires_at")?;
    ...
    let new_signature = signature::sign_manifest_multi(&manifest_bytes, &[(key_id, &private)]);
    if envelope
        .signatures
        .iter()
...
fn verify_release(args: &Arguments) -> Result<()> {
    ...
    let manifest_name = app.metadata_filename();
    let signatures_name = app.signature_filename();
    if manifest_path.file_name().and_then(|name| name.to_str()) != Some(manifest_name.as_str()) {
        return Err(format!("manifest must be named {manifest_name}"));
    }
    if signatures_path.file_name().and_then(|name| name.to_str()) != Some(signatures_name.as_str())
    {
    ...
    signature::verify_manifest_with_keys(&manifest_bytes, &signatures_bytes, &keys.keys)
        .map_err(|error| error.to_string())?;
    manifest
        .ensure_not_expired(Utc::now())
        .map_err(|error| error.to_string())?;
    verify_artifacts(&app, &manifest, artifacts_dir)?;
...
fn verify_artifacts(app: &App, manifest: &ReleaseManifest, directory: &Path) -> Result<()> {
    if !directory.is_dir() {
        return Err(format!(
    ...
    for (target, expected_asset) in &manifest.targets {
        ...
        let bytes = fs::read(&path).map_err(|error| format!("read {}: {error}", path.display()))?;
        release_archive::inspect_archive(app, &bytes, expected_asset)
...
fn validate_published_at(value: &str) -> Result<()> {
    validate_timestamp(value, "published_at")
}
...
fn validate_timestamp(value: &str, field: &str) -> Result<()> {
    ...
    if canonical != value {
        return Err(format!(
            "{field} must be canonical UTC RFC3339 with second precision: {value}"
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    fn canonical_names_reject_wrong_windows_format() {
        ...
        let path = Path::new("demo-1.2.3-x86_64-pc-windows-msvc.tar.gz");
